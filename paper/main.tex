...
The cross-dataset results in Table~\ref{tab:cross} are informative in two directions. When the model is trained exclusively on CIC and evaluated on Trad (CV1), AUC drops to $0.72$ and FNR collapses to $0.00$---the classifier shifts toward predicting nearly every sample as phishing. The model does not lose all discrimination ability (AUC well above chance), but the decision threshold calibrated on CIC does not transfer to the lower-resolution binary matrices of Trad. The reverse direction (CV2) is more severe: training exclusively on Trad and testing on CIC yields near-random performance (AUC $0.52$). A backbone optimized on $69 \times 69$ binary matrices simply has not seen the visual statistics of variable-resolution grayscale PNGs and cannot extrapolate.

Together these results motivate our combined-training design (CV3), under which Q-Shield attains AUC $0.9254$. The diversity of the combined corpus is not a convenience but a necessity: neither dataset alone is sufficient to induce a representation that generalizes to the other. This finding also cautions against reporting quishing detection figures on any single dataset in isolation.
...
\textbf{False negative rate.} The model's FNR of 0.166 remains above the $\leq 0.10$ threshold typically expected in security-critical applications: roughly one in six phishing QR codes is misclassified as benign. Focal loss reduced FNR substantially compared to BCE baselines, but did not fully close the gap. Cost-sensitive threshold calibration and ensembling are natural directions for further improvement.
